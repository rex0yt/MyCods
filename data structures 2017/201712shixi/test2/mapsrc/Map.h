...
public:
  ...
  vector<PersonNo> getAllRelation(int v1);//返回所有相邻边的
...
template <class V, class E>
int Map<V,E>::getWeight(int v1, int v2)
{
  if(v1!=-1&&v2!=-1)
    {
      ...
      while(p!=NULL&&p->dest!=v2)
        p = p->link;
      if(p!= NULL) return p->cost.weight;
    }
  return 0;
}

template<class V, class E>
vector<PersonNo> Map<V,E>::getAllRelation(int v1)
{
  vector<PersonNo> ans;
  Edge<V,E> *pnt = NodeTable[v1].adj;
  while(pnt != NULL)
    {
      ans.push_back(this->getValue(pnt->cost.per1.getNo()));
    }
  return ans;
}
